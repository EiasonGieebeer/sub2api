...
检查账户余额、订阅状态、周期额度和 API Key 自身额度。订阅额度与充值余额可能采用不同的结算范围。需要充值或购买套餐时进入[「充值/订阅」](https://sub.fastapi.cool/purchase)，支付记录在[「我的订单」](https://sub.fastapi.cool/orders)查看。
...
调用 `GET /v1/models`，或查看控制台[「可用渠道」](https://sub.fastapi.cool/available-channels)。不要假设某个上游模型一定已经开放。

## 如何查看服务是否异常？

先看[「渠道状态」](https://sub.fastapi.cool/monitor)，再结合使用记录里的状态码、请求时间和请求 ID 判断。`5xx` 或超时不一定是密钥问题，可能是上游账号、渠道或网络临时异常。
